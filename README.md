...
Untrusted input is a receipt OCR; it can only ever become one of a fixed set of decisions over a
...
- **Untrusted input:** `fetch<web>` — a receipt OCR
- **Closed type:** `type Decision = Reimburse(Category) | Deny` where `Category` is closed
- **Sink / capability:** `grant reimburse irreversible` → `commit { reimburse(d) }`
- **Consequence axes:** Trust, Reversibility

## Run the demo

```sh
examples/apps/expense-reimbursement/demo.sh
```
...
- `expense-reimbursement_safe.wave` — the correct design.
- `expense-reimbursement_unsafe.wave` — the tempting-but-wrong version (the negative example a model must learn to reject).
